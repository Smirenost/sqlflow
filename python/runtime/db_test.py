...
def execute(driver, conn, statement):
    ...
    if driver == "hive":
        ...
        field_names = (
            None if cursor.description is None else [i[0] for i in cursor.description]
        )
...
class TestDB(TestCase):
    ...
    hive_create_statement = (
        "create table test_db (features string, "
    ...
    def test_mysql(self):
        ...
        user, password, host, port, database, _ = parseMySQLDSN(testing.get_mysql_dsn())
        conn = connect(
            driver, database, user=user, password=password, host=host, port=port
        )
    ...
    def test_hive(self):
        ...
        self._do_test(
            driver, conn, hdfs_namenode_addr="127.0.0.1:8020", hive_location="/sqlflow"
        )
        conn.close()
    ...
    def _do_test_hive_specified_db(
        self, driver, conn, hdfs_namenode_addr="", hive_location=""
    ):
...
class TestGenerator(TestCase):
    create_statement = (
        "create table test_table_float_fea " "(features float, label int)"
    )
    drop_statement = "drop table if exists test_table_float_fea"
    insert_statement = (
        "insert into test_table_float_fea (features,label)" " values(1.0, 0), (2.0, 1)"
    )
    ...
    def test_generator(self):
        ...
        user, password, host, port, database, _ = parseMySQLDSN(testing.get_mysql_dsn())
        conn = connect(
            driver, database, user=user, password=password, host=host, port=int(port)
        )
        # prepare test data
        execute(driver, conn, self.drop_statement)
        execute(driver, conn, self.create_statement)
        ...
        gen = db_generator(conn, "SELECT * FROM test_table_float_fea", label_meta)
        idx = 0
        for row, label in gen():
            features = read_features_from_row(
                row, ["features"], ["features"], column_name_to_type
            )
            d = (features, label)
            if idx == 0:
    ...
    def test_generate_fetch_size(self):
        label_meta = {"feature_name": "label", "shape": [], "delimiter": ""}
        gen = db_generator(
            testing.get_singleton_db_connection(),
...
class TestConnectWithDataSource(TestCase):
    ...
    def test_parse_hive_dsn(self):
        self.assertEqual(
            ...
            parseHiveDSN(
                "usr:pswd@hiveserver:1000/mydb?auth=PLAIN&"
        ...
        self.assertEqual(
            ...
            parseHiveDSN(
                "usr:pswd@hiveserver:1000/my_db?auth=PLAIN&"
                "session.mapreduce_job_quenename=mr"
            ),
        )
        self.assertEqual(
    ...
    def test_kv_feature_column(self):
        ...
        raw_val = "0:1 3:4 4:6"
        indices, values, shape = read_feature(
            raw_val, feature_spec, feature_spec["name"]
        )
        self.assertTrue(np.array_equal(indices, np.array([0, 3, 4], dtype=int)))
...
class TestMySQLFieldType(TestCase):
    @unittest.skipUnless(os.getenv("SQLFLOW_TEST_DB") == "mysql", "run only in mysql")
    def test_field_type(self):
        ...
        for int_type, str_type in MYSQL_FIELD_TYPE_DICT.items():
            ...
            self.assertEqual(int_type_actual, int_type, "%s not match" % str_type)


class TestLimitSelect(TestCase):
    def test_limit_select(self):
        self.assertEqual(
            "SELECT * FROM t LIMIT 2", limit_select("SELECT * FROM t LIMIT 30", 2)
        )
        ...
        self.assertEqual("SELECT * FROM t LIMIT 3", limit_select("SELECT * FROM t", 3))

        self.assertEqual(
            "SELECT * FROM t \t  LIMIT 4; ", limit_select("SELECT * FROM t \t ; ", 4)
        )
